...
export const renderCursor = (string, cursor) => {
    if (cursor <= string.length - 1) {
        ...
        return `${string.slice(0, cursor)}${chalk.inverse(string.slice(cursor, cursor + 1))}${string.slice(cursor + 1, string.length)}`;
    }
    else {
...
export const useCursor = ({ value, setValue, focus, cursorPosition, }) => {
    ...
    useInput((input, key) => {
        ...
        if (key.rightArrow || input === "OC") {
            ...
            if (cursor > 0) {
                setValue(`${value.slice(0, cursor - 1)}${value.slice(cursor, value.length)}`);
                setCursor(cursor - 1);
            }
        }
        else if (input !== "\r") {
...
const TextArea = ({ value, setValue, focus, cursorPosition, textProps, }) => {
    ...
    return _jsx(Text, { ...textProps, children: valueWithCursor });
};
